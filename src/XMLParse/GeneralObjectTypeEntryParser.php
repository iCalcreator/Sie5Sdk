...
class GeneralObjectTypeEntryParser extends Sie5ParserBase
{
    /**
     * Parse
     *
     * @return GeneralObjectTypeEntry
     * @throws Exception
     */
    public function parse() : GeneralObjectTypeEntry
    {
        $generalObjectTypeEntry = GeneralObjectTypeEntry::factory()->setXMLattributes( $this->reader );
        $this->logger->debug(
        ...
        if( $this->reader->hasAttributes ) {
            ...
            while( $this->reader->moveToNextAttribute()) {
                ...
                switch( $this->reader->name ) {
                    case ( self::ID ) :
                        $generalObjectTypeEntry->setId( $this->reader->value );
                        break;
                    case ( self::NAME ) :
                        $generalObjectTypeEntry->setName( $this->reader->value );
                        break;
                    case self::XSITYPE :
            ...
            if( isset( $extensionAttributes[self::XSITYPE] ) && ( 2 <= count( $extensionAttributes ))) {
                ...
                $generalObjectTypeEntry->setExtensionAttributes( $extensionAttributes );
            }
