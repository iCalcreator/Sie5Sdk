...
class CustomerInvoiceTypeParser extends Sie5ParserBase
{
    /**
     * Parse
     *
     ...
     * @throws Exception
     */
    public function parse() : CustomerInvoiceType
    {
        $customerInvoiceType = CustomerInvoiceType::factory()->setXMLattributes( $this->reader );
        $this->logger->debug(
        ...
        if( $this->reader->hasAttributes ) {
            ...
            while( $this->reader->moveToNextAttribute()) {
                ...
                switch( $this->reader->name ) {
                    case ( self::ID ) :
                        $customerInvoiceType->setId( $this->reader->value );
                        break;
                    case ( self::NAME ) :
                        $customerInvoiceType->setName( $this->reader->value );
                        break;
                    case ( self::CUSTOMERID) :
                        $customerInvoiceType->setCustomerId( $this->reader->value );
                        break;
                    case ( self::INVOICENUMBER ) :
                        $customerInvoiceType->setInvoiceNumber( $this->reader->value );
                        break;
                    case ( self::OCRNUMBER ) :
                        $customerInvoiceType->setOcrNumber( $this->reader->value );
                        break;
                    case ( self::DUEDATE ) :
                        try {
                            $customerInvoiceType->setDueDate( new DateTime( $this->reader->value ));
                        }
                        catch( Exception $e ) {
                            $this->logger->error(
            ...
            if( isset( $extensionAttributes[self::XSITYPE] ) && ( 2 <= count( $extensionAttributes ))) {
                ...
                $customerInvoiceType->setExtensionAttributes( $extensionAttributes );
            }
