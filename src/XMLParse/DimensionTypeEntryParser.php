...
class DimensionTypeEntryParser extends Sie5ParserBase
{
    /**
     * Parse
     *
     ...
     */
    public function parse() : DimensionTypeEntry
    {
        $dimensionTypeEntry = DimensionTypeEntry::factory()->setXMLattributes( $this->reader );
        $this->logger->debug(
        ...
        if( $this->reader->hasAttributes ) {
            while( $this->reader->moveToNextAttribute()) {
                ...
                switch( $this->reader->name ) {
                    case ( self::ID ) :
                        $dimensionTypeEntry->setId((int) $this->reader->value );
                        break;
                    case ( self::NAME ) :
                        $dimensionTypeEntry->setName( $this->reader->value );
                        break;
                } // end switch
            } // end while
        ...
        while( @$this->reader->read()) {
            ...
            switch( true ) {
                ...
                case ( self::OBJECT == $this->reader->localName ) :
                    $dimensionTypeEntry->addObject( $parser->parse());
                    break;
            } // end switch
        } // end while
        return $dimensionTypeEntry;
