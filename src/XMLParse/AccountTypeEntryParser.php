...
class AccountTypeEntryParser extends Sie5ParserBase
{
    /**
     * Parse
     *
     ...
     */
    public function parse() : AccountTypeEntry
    {
        $accountTypeEntry = AccountTypeEntry::factory()->setXMLattributes( $this->reader );
        $this->logger->debug(
        ...
        if( $this->reader->hasAttributes ) {
            ...
            while( $this->reader->moveToNextAttribute()) {
                ...
                switch( $this->reader->name ) {
                    case ( self::ID ) :
                        $accountTypeEntry->setId( $this->reader->value );
                        break;
                    case ( self::NAME ) :
                        $accountTypeEntry->setName( $this->reader->value );
                        break;
                    case ( self::TYPE ) :
                        $accountTypeEntry->setType( $this->reader->value );
                        break;
                    case ( self::UNIT ) :
                        $accountTypeEntry->setUnit( $this->reader->value );
                        break;
                    case self::XSITYPE :
            ...
            if( isset( $extensionAttributes[self::XSITYPE] ) && ( 2 <= count( $extensionAttributes ))) {
                ...
                $accountTypeEntry->setExtensionAttributes( $extensionAttributes ); // will not work here ...
            }
        ...
        $parser      = new BudgetTypeParser( $this->reader );
        while( @$this->reader->read()) {
            ...
            switch( true ) {
                ...
                case ( self::BUDGET === $this->reader->localName ) :
                    $accountTypeEntry->addBudget( $parser->parse());
                    break;
            } // end switch
        } // end while
        return $accountTypeEntry;
