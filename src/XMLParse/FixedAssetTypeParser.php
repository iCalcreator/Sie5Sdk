...
class FixedAssetTypeParser extends Sie5ParserBase
{
    /**
     * Parse
     *
     ...
     * @throws Exception
     */
    public function parse() : FixedAssetType
    {
        $fixedAssetType = FixedAssetType::factory()->setXMLattributes( $this->reader );
        $this->logger->debug(
        ...
        if( $this->reader->hasAttributes ) {
            ...
            while( $this->reader->moveToNextAttribute()) {
                ...
                switch( $this->reader->name ) {
                    case self::ID :
                        $fixedAssetType->setId( $this->reader->value );
                        break;
                    case self::NAME :
                        $fixedAssetType->setName( $this->reader->value );
                        break;
                    case self::XSITYPE :
            ...
            if( isset( $extensionAttributes[self::XSITYPE] ) && ( 2 <= count( $extensionAttributes ))) {
                ...
                $fixedAssetType->setExtensionAttributes( $extensionAttributes );
            }
        ...
        while( @$this->reader->read()) {
            ...
            switch( true ) {
                ...
                case (self::ORIGINALAMOUNT == $this->reader->localName ) :
                    $fixedAssetType->setOriginalAmount(
