...
class BudgetMultidimTypeWriter extends Sie5WriterBase implements Sie5WriterInterface
{
    ...
    public function write( BudgetMultidimType $budgetMultidimType ) {
        ...
        parent::writeAttribute( $this->writer, self::AMOUNT,   $budgetMultidimType->getAmount());
        parent::writeAttribute( $this->writer, self::QUANTITY, $budgetMultidimType->getQuantity());

        $writer = new ObjectReferenceTypeWriter( $this->writer );
        foreach( $budgetMultidimType->getBudgetMultidimTypes() as $value ) {
            $writer->write( $value );
        }
