...
class OriginalAmountTypeWriter extends Sie5WriterBase implements Sie5WriterInterface
{
    ...
    public function write( OriginalAmountType $originalAmountType ) {
        ...
        parent::writeAttribute( $this->writer, self::AMOUNT, $originalAmountType->getAmount());

        $foreignCurrencyAmount = $originalAmountType->getForeignCurrencyAmount();
        if( ! is_null( $foreignCurrencyAmount )) {
            ForeignCurrencyAmountTypeWriter::factory( $this->writer )->write( $foreignCurrencyAmount );
        }
